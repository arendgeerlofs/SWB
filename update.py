...
def update_states(model):
    ...
    fin_hist, nonfin_hist, SWB_hist = model.fin_hist, model.nonfin_hist, model.SWB_hist
    RFC_hist, soc_cap_hist = model.RFC_hist, model.soc_cap_hist
    N = model.constants['N']
    ...
    RFC_SWB_change = 1 * ((1/np.log(2)) * np.log(RFC_rel+1)-1)
    fin_SWB_change = 1 * ((1/np.log(2)) * np.log(fin_rel+1)-1) * fin_sens_factor
